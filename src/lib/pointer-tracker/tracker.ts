...
import type { PRESS_LIMIT } from '$lib/browser/const.js';
import type { POINT, POINT_WITH_SCALE, SIZE, END_LISTENER } from '$lib/utils/const.js';
	...
	listener = (node: T) => {
		...
		function _pointerStart(event: PointerEvent | MouseEvent) {
			...
			if (isPointerEvent(event)) {
				...
				(capturingElement as Element).setPointerCapture(event.pointerId);
